...
## 当前状态（2026-09-03，BREAKTHROUGHS 顶部 = 续242）
...
- **静态/结构层 100% 收口**（续230-232 终审）：142 原始文件 = 56 CRACKED_NONIMAGE 全 PASS + 33 IMAGE_EXEMPT + 53 RUNTIME_ASSET + 0 UNKNOWN；自测套件 `_run_all_selfchecks.py` PASS（**续242 后 = 165 ref 全 PASS / 0 FAIL**；套件计数按 ref 文件数，勿按断言数估）。
- 最近主破（顶部条）：续239/240/241 技能读·写·消费三侧闭合；**续242 店铺主人格记录表闭合**——`0x517850` 30×12B（=S8 台词表/NPC 师父池同表）+ 入店分发器 `0x44e710`（`[0x52063c]`=0x517850+id*12，存储站全集恰 4 处）+ 12 槽设施身份 msg 锚全识别 + **🔴 纠偏：`+0x07`=店主好感（非「商人资本」）**，学做生意门 40/70/100 = 好感折扣 + 闇商人 #29 事件流 + `0x44e110`=通用持有物选择对话框（-1=取消）。GAME_DATA_SPEC 新增 §3.25；ref `shop_npc_record_ref.py` 80/80。
...
- **非图像结构层零真敞口**（续231/232 终审确证 56/56 PASS）。仅剩 **emu 运行期增强**项（均数值/数据、非逻辑缺口）：
  ...
  - 技能读/写/消费三侧均已闭（读=续239、写=续240、消费归位=续241 完整事件表）；9 音效 ID 中 15(ZANSYU)/36(MATISIRO) 语义中置信（续226）。
  - 续242 闭：`0x44e540` 薄包装（21 调用方逐一语义属数值层）与 `0x44e110`（=通用持有物选择，已闭合）；`+0x07`=店主好感（已坐死）。剩余：记录 +0x08 word/+0x0a/+0x00..06 精确语义、slot3/4/10 内部数值玩法——数值层。
