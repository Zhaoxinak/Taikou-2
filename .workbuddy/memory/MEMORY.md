...
## 当前状态（2026-09-03 双轨并集收口，git main = 3c02e73）
- **双轨并行合并完成**：origin 轨续224-237 + 本地轨续224-227（六类顾问咨询消费链/0x462fd0 typekey map/leaf schema）**并集共存入库**（撞号不重排）；SNDATA_SPEC B 轨三节顺延 §4.0.13-4.0.15；BREAKTHROUGHS 顶部 = 续237。
- **静态/结构层 100% 收口**（续230-232 终审）：142 原始文件 = 56 CRACKED_NONIMAGE 全 PASS + 33 IMAGE_EXEMPT + 53 RUNTIME_ASSET + 0 UNKNOWN；自测套件 `_run_all_selfchecks.py` **152 PASS / 0 FAIL**。
- 最近主破（顶部条）：续236 S7 写入路径（`+0x04`=16位值非指针链、`+0x0c`=守城度非3标志位、`+0x0f`=类别bits4-6+标志bits0-3）；续237 技能 2-bit 位打包（名表序=id 序，位布局 `level_k=(byte[0x1b+k//4]>>((k&3)*2))&3`）+ 纠正「算用/军学」旧述=算术/兵法。
...
- **非图像结构层零真敞口**（续231/232 终审确证 56/56 PASS）。仅剩 **emu 运行期增强**项（均数值/数据、非逻辑缺口）：
  ...
  - `0x518588` 20×5×5 word 矩阵逐格值：结构已闭（续225），值运行期由写回器 `0x4a0ff0/0x4a1010/0x4a1030` 填，须 emu 钩取。
  ...
  - 运行时 entity 技能 getter 写入路径（entity `+0x0f..+0x1d` 2-bit，续237 仍未知①）；9 音效 ID 中 15(ZANSYU)/36(MATISIRO) 语义中置信（续226）。
