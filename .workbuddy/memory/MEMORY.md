...
## 当前状态（2026-09-03，git main = 22e5fd5+，BREAKTHROUGHS 顶部 = 续238）
...
- **静态/结构层 100% 收口**（续230-232 终审）：142 原始文件 = 56 CRACKED_NONIMAGE 全 PASS + 33 IMAGE_EXEMPT + 53 RUNTIME_ASSET + 0 UNKNOWN；自测套件 `_run_all_selfchecks.py` PASS（续238 后含新 ref，见仓库实测计数）。
- 最近主破（顶部条）：续236 S7 写入路径；续237 技能 2-bit 位打包 + 名表「算用/军学」纠错=算术/兵法；**续238 S13 矩阵段文件侧终审**（0x47ef00 装载几何 20×114B@pitch139、总表基址纠偏 0x5185b6→**0x518588**=rec0 blockB 起点 0x5185ba 的近似、两剧本 S13 段 2280B 全 0xFF 占位、静态解密 `[0x58C..EOF)` 校验和 b84b/8013 全中）。
