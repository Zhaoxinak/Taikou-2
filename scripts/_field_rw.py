# -*- coding: utf-8 -*-
"""_field_rw.py — 在「引用某表基址的函数」内，穷举给定字段偏移的全部 读/写 内存访问。
用法: python scripts/_field_rw.py <表基址> <offs...>
仅报告那些函数体内、以某寄存器为基址且 disp 命中目标偏移的访问（不限符号追踪，故能抓到 init 的写）。
"""
...
def main():
    ...
    for fn in fl:
        ...
        for ins in disasm_fn(fn, nxt - fn):
            for o in ins.operands:
                ...
                continue
            break
    print(f"表 {tbl:#x}: {len(sites)} 个函数引用; 扫描偏移 {[hex(t) for t in targets]}")
    ...
    for fn in sorted(sites):
        ...
        for ins in disasm_fn(fn, nxt - fn):
            for o in ins.operands:
                if o.type == CS_OP_MEM and o.mem.base and o.mem.index == 0 and o.mem.disp in targets:
                    m = ins.mnemonic
                    is_w = (m in WR_MNEM and ins.operands and ins.operands[0].type == CS_OP_MEM)
                    hits[o.mem.disp].append((ins.address, m, ins.op_str, 'W' if is_w else 'R'))
    for t in targets:
        print(f"\n=== +{t:#05x} ({t}) : {len(hits[t])} 处 ===")
        for addr, m, ops, rw in sorted(hits[t]):
            print(f"  0x{addr:x}: {m} {ops}   [{rw}]")
    return 0
