...
for TGT in (0x4b9c10, 0x451860):
    fs=enclosing(TGT)
    nxt=None
    for f in FUNCS_S:
        if f> TGT-BASE: nxt=f; break
    size=(nxt-(TGT-BASE)) if nxt else 0x2000
    print(f"\n===== 0x{TGT:06x} (fn 0x{fs+BASE:06x}, {size}B) — type/dispatch/loop scan =====")
    nins=0
    for ins in disasm(TGT,size):
        ...
        if ins.mnemonic in ("cmp","test") and any(c in os_ for c in ("0x341","833","0x172","370")): mark+=" <<CMP-COUNT"
        if "0x31" in os_ or "0x2f" in os_ or "0x1f" in os_: mark+=" <<STRIDE"
        print(f"  0x{ins.address:06x}: {ins.mnemonic} {os_}{mark}")
        if nins>400: 
            print("  ...(truncated at 400 insns)")
            break
