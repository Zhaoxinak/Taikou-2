...
##    headless 用 dummy 渲染驱动，帧率数字无意义。见 progress 表 显示适配 备注。
...
func _finish(t0: int) -> void:
	print("")
	if _fails > 0:
		push_error("[显示适配 FAIL] %d 项断言失败" % _fails)
		quit(1)
	print("[ok] 显示适配 无头可测部分通过（布局回归 + 构建基线，用时 %.0f ms）"
