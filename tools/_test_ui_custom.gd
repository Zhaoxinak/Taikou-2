# _test_ui_custom.gd — 自绘 UI + 多分辨率适配 无头校验（extends SceneTree）
#
# 覆盖：UiTheme 设计空间/缩放比/档位、UiButton 自绘状态机与点击派发、
#       UiPanel 构造、标题画面改用自绘控件后的链路。
# 运行：Godot_v4.7.1/Godot_v4.7.1-stable_win64_console.exe --headless --script res://tools/_test_ui_custom.gd
...
func _test_converted_screens() -> void:
	...
	# 15 = 10 技能 + 休养 + 执行主命 + 回标题 + 外出（大地图）+ 事件弹窗关闭钮（弹窗预置常驻）
	check(ub[0] == 15, "状态画面含 15 个自绘按钮 (10 技能+休养+执行主命+回标题+外出+弹窗关闭钮, got %d)" % ub[0])
...
func _run() -> void:
	...
	if _fails == 0:
		...
		push_error("[FAIL] %d 项断言失败 ❌" % _fails)
		quit(1)
